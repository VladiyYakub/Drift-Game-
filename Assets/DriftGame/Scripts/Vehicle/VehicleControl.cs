...
public class VehicleControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (AIControl.Instance.ControlMode == ControlMode.Mobile)
            {
                if (GameUI.Manage.PanelsMenu.gamePlay.ButtonsUI.activeSelf)
                {
                    steer = Mathf.MoveTowards(steer, steerAmount, Time.deltaTime * (10.0f * PlayerPrefs.GetFloat("Sensitivity")));
                }
                else if (GameUI.Manage.PanelsMenu.gamePlay.AccelUI.activeSelf)
                {
    ...
    void FixedUpdate()
    {
        ...
        if (VehicleMode == VehicleMode.Player && GameUI.Manage.GameStarted)
        {
            if (AIControl.Instance.ControlMode == ControlMode.Simple)
            {
                accel = 0;
                brake = false;
                shift = false;
            ...
            else if (AIControl.Instance.ControlMode == ControlMode.Mobile)
            {
